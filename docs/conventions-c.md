# C / OpenCL Conventions

Applies to all C/H/CL files in packages `core`, `cfgparse`, `sim-ref`,
`sim-gpu`, and `sim-wasm`.

## Naming

- **Files and directories**: `snake_case.c`, `snake_case.h`. Package directory
  names may use hyphens (`sim-gpu`, `sim-ref`) — they are not C identifiers.
- **Public API symbols**: `biosim_` prefix — e.g. `biosim_genome_mutate`, not
  `mutate_genome`.
- **Types**: `biosim_*_t` suffix — e.g. `biosim_coord_t`, `biosim_gene_t`.
- **OpenCL kernel entry points**: `k_` prefix — e.g. `k_feedforward`,
  `k_movement`. No other scope prefix (`s_`, `g_`, etc.) anywhere in the
  codebase.
...
- `assert` is permitted only for invariants that indicate a programming bug —
  not for recoverable runtime conditions.
...
1. `memset(out, 0, sizeof(*out))` — zero the output struct before any
   allocation, so that the free function can safely call `free(NULL)` on any
   uninitialized pointer.
2. Sequential allocations, each guarded by
   `if (ptr == NULL) { returncode = BIOSIM_ERR_NOMEM; goto exit; }`.
3. A single `exit:` label at the end. If `returncode != BIOSIM_OK`, call the
   matching free function (which handles partial initialization via NULL checks),
   then log the error.
4. Free functions must tolerate NULL on every member and NULL on the struct
   itself.

## Error logging discipline

- Functions that exclusively return `BIOSIM_ERR_NOMEM` do **not** log —
  allocation failures are reported by the first caller that has broader context.
...
- Callers exhaustively log OpenCL errors returned from `cl*` callees, using
  `BIOSIM_ERRORF("cl... failed (OpenCL %d)", (int)err)` and return
  `BIOSIM_ERR_OPENCL`. Macros in `cl_macros.h` help reduce boilerplate.
...
Headers with inline functions that are also device-portable (`rng.h`, `gene.h`)
follow their own domain naming — the `_defs` suffix specifically means
"type/enum/constant definitions only, no functions."

### Portability requirements

Shared headers must compile as both C11 and OpenCL C:

- No `<stdio.h>`, `<stdlib.h>`, `<string.h>`, or any other host-only standard
  header.
- No heap allocation, no function pointers, no host struct types with pointer
  members.
- Fixed-width integer types defined via the portability guard in `grid_defs.h`:
...
Four mistakes are easy to make and hard to spot. They break the build on
...
1. **Host-only includes in shared headers.** Headers consumed by OpenCL kernels
   ...
   `core/rng.h`, `core/gene.h`) must not include `<stdio.h>`, `<stdlib.h>`,
   `<string.h>`, or any other host-only standard header. Every such header
   carries a prologue comment flagging the constraint — respect it.
...
4. **Breaking the Windows build.** The repository must build on both
   ...
   POSIX-only headers (`<unistd.h>`, `<sys/types.h>`) in files that are not
   platform-gated; GCC/Clang pragmas or `__attribute__` without an MSVC guard;
   compiler flags added directly to `CMakeLists.txt` instead of
   `cmake/CompilerWarnings.cmake`. See pitfall 2 for flag portability.
...
- `packages/cfgparse/src/bar.c` → `packages/cfgparse/tests/test_bar.c`
...
`tests/CMakeLists.txt` and linked against `unity` and the package under test.

Shared test helpers live in
`packages/core/tests/sim_test_utils.{c,h}` — add to this file when multiple
test modules need the same setup logic.

...
void test_full_config_loads_five_specs(void) {
    ...
    TEST_ASSERT_EQUAL_INT(BIOSIM_BARRIER_CORNER, specs[4].kind);
    TEST_ASSERT_EQUAL_FLOAT(0.3F, specs[4].x);
    TEST_ASSERT_EQUAL_FLOAT(0.3F, specs[4].y);
    TEST_ASSERT_EQUAL_FLOAT(0.2F, specs[4].length);
    TEST_ASSERT_EQUAL_FLOAT(0.02F, specs[4].width);
    TEST_ASSERT_EQUAL_INT(BIOSIM_CORNER_SW, specs[4].quadrant);
...
void test_partial_config_sentinels(void) {
    ...
    /* quadrant defaults to NE when omitted */
    TEST_ASSERT_EQUAL_INT(BIOSIM_CORNER_NE, specs[0].quadrant);
...
int main(void) {
    UNITY_BEGIN();
    RUN_TEST(test_full_config_loads_five_specs);

...
biosim_status_t biosim_grid_create(int32_t size_x, int32_t size_y, biosim_grid_t *out) {
    assert(out != NULL);
    assert(size_x > 0 && size_y > 0);

    uint32_t *cells = calloc((size_t)size_x * (size_t)size_y, sizeof(uint32_t));
    if (!cells) {
...
void biosim_grid_zero_fill(biosim_grid_t *grid) {
    ...
    memset(grid->cells, 0, (size_t)grid->size_x * (size_t)grid->size_y * sizeof(uint32_t));
}
...
uint32_t biosim_grid_at(const biosim_grid_t *grid, biosim_coord_t coord) {
    assert(biosim_grid_in_bounds(grid, coord));
    return grid->cells[(size_t)coord.y * (size_t)grid->size_x + (size_t)coord.x];
}

void biosim_grid_set(biosim_grid_t *grid, biosim_coord_t coord, uint32_t value) {
    assert(biosim_grid_in_bounds(grid, coord));
...
bool biosim_grid_is_occupied(const biosim_grid_t *grid, biosim_coord_t coord) {
    uint32_t cell = biosim_grid_at(grid, coord);
...
biosim_status_t biosim_grid_find_empty(
    ...
    for (int i = 0; i < 200; i++) {
        ...
        if (grid->cells[idx] == BIOSIM_GRID_EMPTY) {
            out->x = idx % grid->size_x;
            out->y = idx / grid->size_x;
            return BIOSIM_OK;
        }
    }
...
void biosim_grid_visit_neighborhood(
    ...
    for (int32_t dy = -radius; dy <= radius; dy++) {
        for (int32_t dx = -radius; dx <= radius; dx++) {
            ...
            biosim_coord_t c = {center.x + dx, center.y + dy};

...
biosim_status_t biosim_genome_create(
    uint32_t population, uint16_t max_genes, biosim_genome_t *out
) {
    assert(out != NULL);
    assert(population > 0 && max_genes > 0);

    memset(out, 0, sizeof(*out));
    out->population = population;
    out->max_genes = max_genes;

    size_t gene_slots = (size_t)max_genes * (size_t)population;
    ...
    out->len = calloc(population, sizeof(uint16_t));
    if (!out->len) {
        returncode = BIOSIM_ERR_NOMEM;
        goto exit;
    }
...
void biosim_genome_init_slot(biosim_genome_t *g, uint32_t idx, uint16_t len, uint64_t *rng) {
    ...
    assert(len > 0 && len <= g->max_genes);

    g->len[idx] = len;
    for (uint32_t j = 0; j < len; j++) {
        size_t slot = (size_t)j * g->population + idx;
...
void biosim_genome_copy_slot(biosim_genome_t *g, uint32_t dst, uint32_t src) {
    ...
    g->len[dst] = g->len[src];
    for (uint32_t j = 0; j < g->max_genes; j++) {
...
void biosim_genome_crossover(
    ...
    uint32_t len_a = g->len[parent_a];
    uint32_t len_b = g->len[parent_b];
    ...
    if (child_len > g->max_genes) {
        child_len = g->max_genes;
    }

    for (uint32_t j = 0; j < k; j++) {
        size_t dst_slot = (size_t)j * g->population + child;
        size_t src_slot = (size_t)j * g->population + parent_a;
        g->conn[dst_slot] = g->conn[src_slot];
        g->wgt[dst_slot] = g->wgt[src_slot];
    }
    for (uint32_t j = k; j < child_len; j++) {
        size_t dst_slot = (size_t)j * g->population + child;
        size_t src_slot = (size_t)j * g->population + parent_b;
        g->conn[dst_slot] = g->conn[src_slot];
        g->wgt[dst_slot] = g->wgt[src_slot];
    }
    g->len[child] = (uint16_t)child_len;
}
...
biosim_status_t biosim_genome_sort_by_length(biosim_genome_t *g, uint32_t *perm_out) {
    ...
    size_t gene_slots = (size_t)g->max_genes * (size_t)pop;
    size_t buckets = (size_t)g->max_genes + 1U;
    ...
    /* Counting sort (descending) on len[0..pop-1] in range [0..max_genes] */
    ...
    memcpy(g->len, new_len, (size_t)pop * sizeof(uint16_t));
exit:
    free(new_conn);
    free(new_wgt);
    free(new_len);

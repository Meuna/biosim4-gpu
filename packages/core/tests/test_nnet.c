...
static void set_gene(
    uint32_t agent, uint16_t j, uint8_t st, uint8_t sn, uint8_t dt, uint8_t dn, int16_t w
) {
...
void test_transposed_conn_indexing(void) {
    ...
    genome.len[1] = 1;
    TEST_ASSERT_EQUAL_INT(
        BIOSIM_OK, biosim_nnet_compile_slot(&nnet, &genome, 0, NUM_SENSORS, NUM_ACTIONS)
    );
    TEST_ASSERT_EQUAL_INT(
        BIOSIM_OK, biosim_nnet_compile_slot(&nnet, &genome, 1, NUM_SENSORS, NUM_ACTIONS)
    );
...
void test_max_conn_not_exceeded(void) {
    ...
    TEST_ASSERT_EQUAL_INT(
        BIOSIM_OK, biosim_nnet_compile_slot(&small, &genome, AGENT_IDX, NUM_SENSORS, NUM_ACTIONS)
    );
...
void test_fingerprint_differs_for_different_nnets(void) {
    ...
    TEST_ASSERT_NOT_EQUAL_UINT64(
        biosim_nnet_fingerprint(&nnet, 0), biosim_nnet_fingerprint(&nnet, 1)
    );
}
